...
import com.arunika.studysmart.util.Constants
		...
		LazyColumn(
			...
			item {
				RelatedToSubjectSection(
					...
					selectSubjectButtonOnClick = { isBottomSheetOpen = true } ,
					seconds = seconds
				)
			}
			item {
...
private fun RelatedToSubjectSection(
	...
	selectSubjectButtonOnClick : () -> Unit ,
	seconds : String
) {
	Column(modifier = modifier) {
		...
		Row(
			...
			IconButton(
				onClick = selectSubjectButtonOnClick ,
				enabled = seconds == "00"
			) {
				Icon(
